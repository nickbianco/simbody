...
//==============================================================================
//                       SCALED SYSTEM JACOBIAN METHODS
//==============================================================================
...
//------------------------------------------------------------------------------
// Ensure that we have contiguous storage and then call the underlying method.
void SimbodyMatterSubsystem::multiplyByScaledSystemJacobian(
    const State& state,
    ...
    Vector_<SpatialVec>& Ju) const
{
    ...
    SimTK_ERRCHK2_ALWAYS(s.size() == nb,
        "SimbodyMatterSubsystem::multiplyByScaledSystemJacobian()",
        "The supplied Vector of scale factors had length %d; expected %d.",
        s.size(), nb);
    ...
    rep.multiplyByScaledSystemJacobian(state, s, *up, *Jup);
...
//------------------------------------------------------------------------------
// Ensure that we have contiguous storage and then call the underlying method.
void SimbodyMatterSubsystem::multiplyByScaledSystemJacobianTranspose(
    const State& state,
    const Vector_<Vec3>& s,
    ...
    Vector& f) const
{
    ...
    SimTK_ERRCHK2_ALWAYS(s.size() == nb,
        "SimbodyMatterSubsystem::multiplyByScaledSystemJacobianTranspose()",
        "The supplied Vector of scale factors had length %d; expected %d.",
        s.size(), nb);
    ...
    rep.multiplyByScaledSystemJacobianTranspose(state, s, *F_Gp, *fp);
...
//------------------------------------------------------------------------------
//                    MULTIPLY BY SCALED STATION JACOBIAN
//------------------------------------------------------------------------------
// We want v_GS = J_GS(q,s)*u, the linear velocity of nt station tasks Si in
// Ground induced by the given generalized speeds for a system scaled by body
// scale factors s. Station Si is on body Bi and is given by the Bi-frame vector
// p_BiSi. We can easily calculate V_GB = J_GB(q,s)*u, the spatial velocity of
// *all* the mobilized bodies. Then for each station task,
//      p_BiSi_scaled_G = p_BiSi_G ⊙ s_Bi
//      v_GSi = v_GBi + w_GBi X p_BiSi_scaled_G
// where p_BiSi_scaled_G is p_BiSi re-expressed in Ground and scaled by the body
// scale factors.
//
// Cost is 30*nt + 162*nb + 57*nu flops.
void SimbodyMatterSubsystem::multiplyByScaledStationJacobian(
    const State&                        state,
    const Vector_<Vec3>&                s,
    const Array_<MobilizedBodyIndex>&   onBodyB,
    const Array_<Vec3>&                 p_BS,
    ...
    Vector_<Vec3>&                      JSu) const
{
    const SimbodyMatterSubsystemRep& rep = getRep();
    const int nb = rep.getNumBodies(), nu = rep.getNumMobilities();
    const int nt = (int)onBodyB.size();

    SimTK_ERRCHK2_ALWAYS(s.size() == nb,
        "SimbodyMatterSubsystem::multiplyByScaledStationJacobian()",
        "The supplied Vector of scale factors had length %d; expected %d.",
        s.size(), nb);
    ...
    Vector_<SpatialVec> Ju(nb);
    if (u.hasContiguousData())
        rep.multiplyByScaledSystemJacobian(state, s, u, Ju);
    else {
        Vector contig_u(nu);
        contig_u(0,nu) = u;
        rep.multiplyByScaledSystemJacobian(state, s, contig_u, Ju);
    }
    ...
    for (int task=0; task < nt; ++task) {
        ...
        SimTK_INDEXCHECK(mobodx, nb,
            "SimbodyMatterSubsystem::multiplyByScaledStationJacobian()");
        const MobilizedBody& mobod = rep.getMobilizedBody(mobodx);
        const Vec3 p_BS_scaled = s[mobodx].elementwiseMultiply(p_BS[task]);
        const Vec3 p_BS_scaled_G =
            mobod.expressVectorInGroundFrame(state, p_BS_scaled);
        const SpatialVec V_GS = shiftVelocityBy(Ju[mobodx], p_BS_scaled_G);
        JSu[task] = V_GS[1];
    }
}
...
//------------------------------------------------------------------------------
//               MULTIPLY BY SCALED STATION JACOBIAN TRANSPOSE
//------------------------------------------------------------------------------
// We want f = ~J_GS(q,s)*f_GS, the generalized forces produced by applying
// translational task force vectors f_GSi to stations Si for a system scaled by
// a set of scale factors s. Each station Si is on body Bi and is given by the
// Bi-frame vector p_BiSi. We can easily calculate f = ~J_GB(q,s)*F_GB for task
// body origins, so we shift the task forces there with,
//      p_BiSi_scaled_G = p_BiSi_G ⊙ s_Bi
//      F_GBi=[p_BiSi_scaled_G X f_GSi, f_GSi].
// where p_BiSi_scaled_G is p_BiSi re-expressed in Ground and scaled by the body
// scale factors.
//
// Cost is 33*nt + 180*nb + 57*nu flops.
void SimbodyMatterSubsystem::multiplyByScaledStationJacobianTranspose(
    const State&                        state,
    const Vector_<Vec3>&                s,
    const Array_<MobilizedBodyIndex>&   onBodyB,
    const Array_<Vec3>&                 p_BS,
    ...
    Vector&                             f) const
{
    ...
    SimTK_ERRCHK2_ALWAYS(s.size() == nb,
        "SimbodyMatterSubsystem::multiplyByScaledStationJacobianTranspose()",
        "The supplied Vector of scale factors had length %d; expected %d.",
        s.size(), nb);
    ...
    for (int task=0; task < nt; ++task) {
        ...
        const Vec3 p_BS_scaled_G =
            mobod.expressVectorInGroundFrame(state, p_BS_scaled);
        F_G[mobodx] += SpatialVec(p_BS_scaled_G % f_GS[task], f_GS[task]);
    }

    rep.multiplyByScaledSystemJacobianTranspose(state, s, F_G, *fp);
...
//------------------------------------------------------------------------------
//                     MULTIPLY BY SCALED FRAME JACOBIAN
//------------------------------------------------------------------------------
// We want V_GA = J_GA(q,s)*u, the spatial velocity of nt task frames Ai in
// Ground induced by the given generalized speeds for a system scaled by a set
// of scale factors s. Frames A are fixed on bodies B and would be given by the
// transform X_BA, except the result depends only on A's origin position p_BA
// (==p_BoAo) because angular velocity is the same for all frames fixed to the
// same body. We can easily calculate V_GB = J_GB(q,s)*u, the spatial velocities
// at each body B's origin Bo. Then
//      p_BiAi_scaled_G = p_BiAi_G ⊙ s_Bi
//      V_GAi = [w_GAi, v_GAi] = [w_GBi, v_GBi + w_GBi X p_BiAi_scaled_G]
// where p_BiAi_scaled_G is p_BiAi re-expressed in Ground and scaled by the body
// scale factors.
//
// Cost is 30*nt + 162*nb + 57*nu flops.
void SimbodyMatterSubsystem::multiplyByScaledFrameJacobian
   (const State&                        state,
    const Vector_<Vec3>&                s,
    const Array_<MobilizedBodyIndex>&   onBodyB,
    const Array_<Vec3>&                 p_BA,
    ...
    Vector_<SpatialVec>&                JFu) const
{
    ...
    SimTK_ERRCHK2_ALWAYS(s.size() == nb,
        "SimbodyMatterSubsystem::multiplyByScaledFrameJacobian()",
        "The supplied Vector of scale factors had length %d; expected %d.",
        s.size(), nb);
    ...
    for (int task=0; task < nt; ++task) {
        ...
        SimTK_INDEXCHECK(mobodx, nb,
            "SimbodyMatterSubsystem::multiplyByScaledFrameJacobian()");
        const MobilizedBody& mobod = rep.getMobilizedBody(mobodx);
        const Vec3 p_BA_G = mobod.expressVectorInGroundFrame(state,
            p_BA[task].elementwiseMultiply(s[mobodx]));
        JFu[task] = shiftVelocityBy(Ju[mobodx], p_BA_G);
    }
}


//------------------------------------------------------------------------------
//                 MULTIPLY BY SCALED FRAME JACOBIAN TRANSPOSE
//------------------------------------------------------------------------------
// We want f = ~J_GA(q,s)*F_GA, the generalized forces produced by applying
// spatial force vectors F_GAi=[t_G,f_GAi] at Aio, the origin of task frame Ai,
// which is fixed to some body Bi. Frame Ai would be given by transform X_BAi,
// but the result depends only on Ai's origin location p_BiAi (==p_BioAio) since
// a torque is the same wherever it is applied. We can easily calculate
// fb = ~J_GB(q,s)*F_GB so we shift each F_GAi to Bi via
//      p_BiAi_scaled_G = p_BiAi_G ⊙ s_Bi
//      F_GBi = [t_G + p_BiAi_scaled_G X f_GAi, f_GAi].
// where p_BiAi_scaled_G is p_BiAi re-expressed in Ground and scaled by the body
// scale factors.
//
// Cost is 33*nt + 180*nb + 57*nu flops.
void SimbodyMatterSubsystem::multiplyByScaledFrameJacobianTranspose(
    const State&                        state,
    const Vector_<Vec3>&                s,
    const Array_<MobilizedBodyIndex>&   onBodyB,
    const Array_<Vec3>&                 p_BA,
    ...
    Vector&                             f) const
{
    ...
    SimTK_ERRCHK2_ALWAYS(s.size() == nb,
        "SimbodyMatterSubsystem::multiplyByScaledFrameJacobianTranspose()",
        "The supplied Vector of scale factors had length %d; expected %d.",
        s.size(), nb);
    ...
    for (int task=0; task < nt; ++task) {
        ...
        const Vec3 p_BA_G = mobod.expressVectorInGroundFrame(state,
            p_BA[task].elementwiseMultiply(s[mobodx]));
        const SpatialVec& F_GAi = F_GA[task];
        F_G[mobodx] += SpatialVec(F_GAi[0] + p_BA_G % F_GAi[1], F_GAi[1]);
    }
...
//------------------------------------------------------------------------------
// We want p_like = JP*s_like, where Jacobian JP = partial(p_GB)/partial(s) and
// p_GB is the position of body B's origin in Ground.
//
// Cost is 75*nb flops.
void SimbodyMatterSubsystem::multiplyByPositionJacobianWrtBodyScales(
    const State& state,
    const Vector_<Vec3>& s,
    Vector_<Vec3>& JPs) const
{
    ...
    SimTK_ERRCHK2_ALWAYS(s.size() == nb,
        "SimbodyMatterSubsystem::multiplyByPositionJacobianWrtBodyScales()",
        "The supplied Vector of scale factor perturbations had length %d; "
        "expected %d.", s.size(), nb);


    const bool sIsContig   = s.hasContiguousData();
    const bool JPsIsContig = JPs.hasContiguousData();

    Vector_<Vec3> s_contig; Vector_<Vec3> JPs_contig; // allocate only if needed
    const Vector_<Vec3>* sp     = sIsContig   ? &s   : &s_contig;
    Vector_<Vec3>*       JPs_ptr = JPsIsContig ? &JPs : &JPs_contig;
    if (!sIsContig) {
        s_contig.resize(nb);
        s_contig(0, nb) = s; // prevent reallocation
    }

    rep.multiplyByPositionJacobianWrtBodyScales(state, *sp, *JPs_ptr);
...
//------------------------------------------------------------------------------
// We want s_like = ~JP*p_like, where Jacobian JP = partial(p_GB)/partial(s) and
// p_GB is the position of body B's origin in Ground.
//
// Cost is 102*nb flops.
void SimbodyMatterSubsystem::multiplyByPositionJacobianWrtBodyScalesTranspose(
    const State& state,
    ...
    Vector_<Vec3>& JPtp) const
{
    ...
    SimTK_ERRCHK2_ALWAYS(p.size() == nb,
        "SimbodyMatterSubsystem::"
        "multiplyByPositionJacobianWrtBodyScalesTranspose()",
        "The supplied vector of body position perturbations had length %d; "
        "expected %d.", p.size(), nb);

    const bool pIsContig    = p.hasContiguousData();
    const bool JPtpIsContig = JPtp.hasContiguousData();

    Vector_<Vec3> p_contig; Vector_<Vec3> JPtp_contig; // allocate only if needed
    const Vector_<Vec3>* p_ptr    = pIsContig    ? &p    : &p_contig;
    Vector_<Vec3>*       JPtp_ptr = JPtpIsContig ? &JPtp : &JPtp_contig;
    if (!pIsContig) {
        p_contig.resize(nb);
        p_contig(0, nb) = p; // prevent reallocation
    }

    rep.multiplyByPositionJacobianWrtBodyScalesTranspose(state, *p_ptr,
                                                         *JPtp_ptr);
...
//------------------------------------------------------------------------------
//                MULTIPLY BY STATION JACOBIAN WRT BODY SCALES
//------------------------------------------------------------------------------
void SimbodyMatterSubsystem::multiplyByStationJacobianWrtBodyScales(
    const State&                        state,
    const Array_<MobilizedBodyIndex>&   onBodyB,
    const Array_<Vec3>&                 p_BS,
    const Vector_<Vec3>&                s,
    Vector_<Vec3>&                      JSs) const
{
    const SimbodyMatterSubsystemRep& rep = getRep();
    const int nb = rep.getNumBodies();
    const int nt = (int)onBodyB.size();
    ...
    SimTK_ERRCHK2_ALWAYS(s.size() == nb,
        "SimbodyMatterSubsystem::multiplyByStationJacobianWrtBodyScales()",
        "The supplied scale perturbation vector had length %d; expected %d.",
        s.size(), nb);
    ...
    // Body-origin displacements for all bodies.
    Vector_<Vec3> dp_body;
    rep.multiplyByPositionJacobianWrtBodyScales(state, s, dp_body);
    ...
    for (int task = 0; task < nt; ++task) {
        ...
        SimTK_INDEXCHECK(mobodx, nb,
            "SimbodyMatterSubsystem::multiplyByStationJacobianWrtBodyScales()");
        const MobilizedBody& mobod = rep.getMobilizedBody(mobodx);
        // Station offset scales with the body: add R_GB*(p_BS ⊙ s_B).
        const Vec3 p_BS_scaled =
                   p_BS[task].elementwiseMultiply(s[mobodx]);
        JSs[task] = dp_body[mobodx] +
                    mobod.expressVectorInGroundFrame(state, p_BS_scaled);
    }
}

//------------------------------------------------------------------------------
//            MULTIPLY BY STATION JACOBIAN WRT BODY SCALES TRANSPOSE
//------------------------------------------------------------------------------
void SimbodyMatterSubsystem::multiplyByStationJacobianWrtBodyScalesTranspose(
    const State&                        state,
    const Array_<MobilizedBodyIndex>&   onBodyB,
    const Array_<Vec3>&                 p_BS,
    const Vector_<Vec3>&                p_GS,
    Vector_<Vec3>&                      JStp) const
{
    const SimbodyMatterSubsystemRep& rep = getRep();
    const int nb = rep.getNumBodies();
    const int nt = (int)onBodyB.size();

    SimTK_ERRCHK3_ALWAYS(p_BS.size() == nt && p_GS.size() == nt,
        "SimbodyMatterSubsystem::"
        "multiplyByStationJacobianWrtBodyScalesTranspose()",
        "The given number of task bodies (%d), task stations (%d), and "
        "station position perturbations (%d) must all be the same.",
        nt, (int)p_BS.size(), (int)p_GS.size());

    // Accumulate the translational displacements dp for each body from the
    // based on the station contributions in p_GS.
    Vector_<Vec3> dp_body(nb, Vec3(0));
    for (int task = 0; task < nt; ++task) {
        SimTK_INDEXCHECK(onBodyB[task], nb,
          "SimbodyMatterSubsystem::"
          "multiplyByStationJacobianWrtBodyScalesTranspose()");
        dp_body[onBodyB[task]] += p_GS[task];
    }

    // Compute the contributions to JStp from body origin perturbations via
    // ~JP*dp_body.
    rep.multiplyByPositionJacobianWrtBodyScalesTranspose(state, dp_body, JStp);

    // Apply the contributions to JStp from each station offset:
    // JStp[B] += (~R_GB * p_GS) ⊙ p_BS.
    for (int task = 0; task < nt; ++task) {
        const MobilizedBodyIndex mobodx = onBodyB[task];
        const MobilizedBody& mobod = rep.getMobilizedBody(mobodx);
        const Rotation& R_GB = mobod.getBodyTransform(state).R();
        const Vec3 dp_BS = ~R_GB * p_GS[task];
        JStp[mobodx] += dp_BS.elementwiseMultiply(p_BS[task]);
    }
}

//------------------------------------------------------------------------------
//                        CALC SCALED STATION POSITION
//------------------------------------------------------------------------------
void SimbodyMatterSubsystem::calcScaledStationPosition(
    const State&                        state,
    const Vector_<Vec3>&                s,
    const Array_<MobilizedBodyIndex>&   onBodyB,
    const Array_<Vec3>&                 p_BS,
    Vector_<Vec3>&                      p_GS) const
{
    ...
    SimTK_ERRCHK2_ALWAYS(s.size() == nb,
        "SimbodyMatterSubsystem::calcScaledStationPosition()",
        "The supplied scale factor vector had length %d; expected %d.",
        s.size(), nb);
    ...
    // Compute the scaled body origin positions by traversing the kinematic
    // tree and applying each mobilizer's scaled transform.
    Vector_<Vec3> p_GB_scaled;
    rep.calcScaledBodyPositions(state, s, p_GB_scaled);

    // Station positions: body origin + scaled station offset in ground.
    p_GS.resize(nt);
    for (int task = 0; task < nt; ++task) {
        const MobilizedBodyIndex b = onBodyB[task];
        SimTK_INDEXCHECK(b, nb,
            "SimbodyMatterSubsystem::calcScaledStationPosition()");
        const MobilizedBody& mobod = rep.getMobilizedBody(b);
        // Station offset in B scales with the body; rotation is unchanged.
        const Rotation& R_GB = mobod.getBodyTransform(state).R();
        p_GS[task] = p_GB_scaled[b] +
                     R_GB * p_BS[task].elementwiseMultiply(s[b]);
    }
}
