...
// only rotational coordinates! However, since F is at the origin of the beam
// and M at the end point, we can at least obtain a translation in
// the *direction* of the requested translation. The magnitude must of
// course be set to end up with the M origin at the end of the beam, and Mz will
// be the tangent to the beam at that point.
//
// Expressed as an X-Y-Z body fixed Euler sequence, the Z rotation is just
// the spin around the Mz (beam tangent) and could be anything, so we'll
